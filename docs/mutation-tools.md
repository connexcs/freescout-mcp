...
| `freescout_send_reply` | Creates a published customer-visible reply and schedules FreeScout's normal delivery workflow. `confirm_send: true` is mandatory on every call. |
| `freescout_create_ticket` | Creates a new email conversation with a published initial message and schedules FreeScout's normal delivery workflow. `confirm_send: true` is mandatory. |
| `freescout_set_ticket_tags` | Replaces the complete tag set on a ticket when the FreeScout Tags module/schema is available. Only existing tag names are accepted. |
...
## Customer-visible confirmation and delivery state
...
Customer-visible operations create the published thread with the selected conversation recipient before invoking FreeScout's standard user-reply/user-created-conversation events and undo/background-action delivery flow. A successful MCP response contains `delivery_state: scheduled` and `sent: false`. This means the conversation/thread mutation was committed and FreeScout delivery was scheduled; it does **not** claim that the downstream mail transport has already delivered the message.
...
`freescout_set_ticket_tags` has replace semantics. It accepts existing tag names only; the MCP server deliberately does not create global tag definitions because permission to update a conversation does not imply permission to manage the global Tags catalogue. Unknown names are rejected.

Tag reads and mutations are exposed only when a compatible Tags schema is detected. Tag reads are constrained to tags associated with conversations visible to the authenticated user, and global tag counters are not exposed.
...
Published replies and newly created conversations follow FreeScout's normal conversation/thread fields, events, folder counters and background-delivery hooks. Mutations, idempotency state, and audit records are transactionally coordinated by the existing mutation executor.
