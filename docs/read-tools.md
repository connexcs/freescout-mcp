...
| `freescout_search_tickets` | Search accessible tickets with optional mailbox, customer, assignee, and status filters. |
...
Ticket results contain operational metadata and a short preview. Thread bodies are converted to plain text and capped at 50,000 characters. Attachment metadata is returned, but attachment contents, storage paths, message headers, BCC recipients, internal model attributes, and mailbox credentials are never returned.
...
- FreeScout's conversation policy is checked again for direct ticket reads.
